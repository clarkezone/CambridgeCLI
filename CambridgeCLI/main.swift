...
class Harness {
    init() {
        //z80 = Z80(memory: Memory(sizeInBytes: 65536), portRead: fportRead, portWrite: fportWrite)
        //z80 = Z80(memory: Memory(sizeInBytes: 65536), portRead: self.portRead, portWrite: self.portWrite)
        z80 = Z80(memory: Memory(sizeInBytes: 65536))
        //z80.onPortRead = self.portRead
        //z80.onPortWrite = self.portWrite
    ...
    func emulate(_ file: URL) {
        ...
        repeat {
            //        printDisassembly()
            _ = z80.executeNextInstruction(portRead:portRead, portWrite:portWrite) //pass in onread here
            total += 1
        } while !isDone
    ...
    func go() {
        ...
        let file = URL(filePath: "/Users/jamesclarke/src/github.com/timsneath/CambridgeCLI/CambridgeCLI/zex/zexdoc.com")
        let start = Date()
        emulate(file)
        let stop = Date()
